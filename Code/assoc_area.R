...
for (i in 1:length(intercept)) {
  ...
  gg <-  ggplot(plotdata, aes(time, y)) + facet_wrap(~ group, scales = 'free_y', nrow = 2, strip.position = "right") + 
    theme_bw() + theme(axis.title.y = element_blank(), axis.text = element_blank(), 
    ...
    geom_point(data = new_plotdata[new_plotdata$group == 'Hazard', ][i, ], 
               aes(x = intercept, y = h_smooth_y), size = 2, alpha = 0.4, color = '#ffb400') + 
  ...
  areadat <- ggplot_build(gg)$data[[3]]
  areadat$group <- levels(plotdata$group)[2]
  areadat$group <- factor(areadat$group, levels = c("Hazard",
                                                    "Longitudinal Outcome"))

  xnew <- ifelse(areadat$x <= intercept[i], areadat$x, NA)
  ynew <- ifelse(is.na(xnew), NA, areadat$y)
  
  areadat$xnew <- xnew
  areadat$ynew <- ynew
  
  new_plotdata$xnew <- xnew
  new_plotdata$ynew <- ynew
  ...
  ggplot(plotdata, aes(time, y)) + facet_wrap(~ group, scales = 'free_y', nrow = 2, strip.position = "right") + 
    ...
    geom_segment(data = new_plotdata[new_plotdata$group == 'Longitudinal outcome', ][i, ], 
                 aes(x = intercept, xend = intercept, y = 0, yend = 2.2), 
                 color = '#2d4059') + 
    facet_grid_sc(rows = vars(group), scales = list(y = myscales)) + 
    geom_blank(data = plotdata[plotdata$group == "Longitudinal outcome", ]) + 
    geom_area(data = new_plotdata[new_plotdata$group == 'Longitudinal outcome', ], 
              aes(xnew, ynew), alpha = 0.2, fill = "#2d4059")

  
  ggsave(filename = paste0('D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\JM_Classic\\White_BG\\assoc_area_', i, '.png'), 
         height = 648, width = 1280, units = 'px')
}

paths <- rep(NA, 80)
for(i in 1:80) {
  paths[i] <- paste0('D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\JM_Classic\\White_BG\\assoc_area_',
                     i, '.png')
}

gif_file <- gifski(paths, delay = 0.05, loop = FALSE, width = 1280, height = 648,
                   gif_file = 'D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\JM_Classic\\White_BG\\animation_assoc_area.gif')
