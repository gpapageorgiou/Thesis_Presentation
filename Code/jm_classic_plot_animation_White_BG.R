...
ggsave(filename = 'D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\JM_Classic\\White_BG\\JMclassic_1.png', 
       height = 648, width = 1280, units = 'px')

sizes = seq(0.25, 3.5, length.out = 15)
...
for (i in 1:15) {
  ...
  #              size = 1)
  ggsave(filename = paste0('D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\JM_Classic\\White_BG\\JMclassic_', i+1, '.png'), 
         height = 648, width = 1280, units = 'px')
}

paths <- rep(NA, 15)
for(i in 1:15) {
  paths[i] <- paste0('D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\JM_Classic\\White_BG\\JMclassic_', 
                     i+1, '.png')
}

gif_file <- gifski(paths, delay = 1, loop = FALSE, width = 1280, height = 648)

sizes_h <- seq(0.3, 1, length.out = 80)
