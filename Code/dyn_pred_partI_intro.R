...
for (i in 1:7) {
  ggplot() + 
    ...
    ylab('Longitudinal Outcome') + xlab('Time')
  
  ggsave(filename = paste0('D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\PART_I\\dyn_pred_intro_', i, '.png'), 
         height = 648, width = 1280, units = 'px')
}

paths <- rep(NA, 7)
for(i in 1:7) {
  paths[i] <- paste0('D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\PART_I\\dyn_pred_intro_', 
                     i, '.png')
}

gif_file <- gifski(paths, delay = 1.5, loop = FALSE, width = 1280, height = 648, 
                   gif_file = 'D:\\gpapageorgiou\\myPresentations\\Thesis_Presentation\\Animations\\PART_I\\dyn_pred_intro.gif')
